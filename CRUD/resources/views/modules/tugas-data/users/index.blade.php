...
<div class="row mt-5">
                    ...
                    <table class="table table-striped table-hover align-middle ">
                        ...
                        <tbody class="fw-6 text-secondary">
                            ...
                            <tr>
                                ...
                                <td> {{ $users->phone }}</td>
                                <td>{{ ($users->gender) ? "M":"F" }}</td>
                                <td>
                                    <div class="btn-group">
                                        <button class="btn btn-sm btn-danger" title="Remove" type="button"
                                            onclick="RemoveItem({{ $users->id }})">
                                            <i class="bi bi-trash"></i>
                                        </button>
                                        <button title="Edit" class="btn btn-sm btn-primary" type="button"
    ...
    <div class="col-lg-4 col-xxl-4">
        @include('modules.tugas-data.users.create')
    </div>
</div>
...
const RemoveItem = (id) => {
    if (confirm("Are you sure wants to remove this user ?")) {
        ...
        xmlhttp.open("GET", "{{ url('') }}/tugas-data/users/remove/" + id);
        xmlhttp.send();
    }
}
const EditItem = (id) => {
    var targetDiv = document.getElementById("form-users");
    ...
    xmlhttp.open("GET", "{{ url('') }}/tugas-data/users/edit/" + id);
    xmlhttp.send();
}
</script>
